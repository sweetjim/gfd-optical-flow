classdef exptdata<handle
    ...
    properties  % Global variables 
        ...
        refframe = 'cartesian' % Reference frame before computation
    end
    ...
    methods % VELOCITY
        function plotVelocity(obj,imageindex,varargin)
            ...
            reframe = 'cartesian';      % Reference frame of output
            ...
            %% Reference frame
            if strcmp(reframe,'polar')
                img = image_preprocessing(img,'polar');
                ux  = image_preprocessing(ux,'polar');
                uy  = image_preprocessing(uy,'polar');
            end
...
%                         ulims=[0, 1];
            ...
            function parseInput(varargin)
                ...
                for k=1:length(items)
                    switch items{m}
                        case 'ax'
                            ax = namevalue;
                        case {'ref','refframe','freference','frame'}
                            reframe = namevalue;
                        case 'input'
                            in = namevalue;
        ...
        function out    = getVelocity(obj,imageindex,varargin)
            ...
            %
            %       previous-   [string]
            %                   Use previous parameter settings if
            %                   available.
            %                   DEFAULT IS OFF.
            %
            %       method  -   [string] 
            ...
            %                           required   argname,argval  argval
            %_________________________________________________________________
            %_________________________________________________________________
            %% Inputs
            dt          = 1;
            ...
            params      = struct;
            if nargin==1
                error('Not enough input arguments. A valid image-index must be provided.')
            end
            if isempty(loop)
                loop = imageindex;
            end
...
%                     strfcn = zeros(size(im1,1),numel(loop));
                ...
                out     = velocity_field(gather(im1),gather(im2), ...
                    method, ...
                    alpha, ...
                    iterations, ...
                    lambda, ...
                ...
                % masking
                if mask
                    out.u = obj.applyMask(out.u,imageindex);
                    out.v = obj.applyMask(out.v,imageindex);
                end
...
%                 out.t = obj.times(loop)-obj.times(1);
            ...
            function checkpreviousstate
                ...
                if isempty(obj.velocity.params)
                    return
                end
                tmp = obj.velocity.params;
                fields = fieldnames(tmp);
                for i=1:numel(fields)
                    command = sprintf('%s=tmp.%s;',fields{i},fields{i});
                    eval(command)
                end

            end
        end
        function [out,mask] = applyMask(obj,in,imageindex)
    ...
    methods % IMAGE LOADING AND DIMENSIONS
        function im = loadImage(obj,imageindex,variable)
            ...
            im = image_preprocessing(im,obj.refframe);
